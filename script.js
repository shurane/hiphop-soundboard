...
function getPlaceholditUrl() {
  return "http://placehold.it/150x100/" + getRandomColor() + "/fff";
}
...
function playSnippet(source, begin, end){
  var audio = new Audio(source);
  ...
  audio.addEventListener("timeupdate", function(){
    console.log("source: ", source, "timeupdate: ", audio.currentTime);
...
for (var i = 0; i < anchors.length; i++) {
  images[i].src = getHolderUrl(clips[i].name);

  anchors[i].addEventListener('click', playClip(clips[i].file));
}
